...
export const UI = {
    ...
    // [틀 고정] 앱 전체의 골격은 여기서 단 한 번만 그립니다.
    init() {
    ...
    // [내용만 교체] 사이드바 클릭 시 제목과 본문 텍스트만 갈아끼웁니다.
    switchView(viewId) {
        this.closeDetail();
        ...
        if (cur.render) {
            ...
            body.innerHTML = `<div class="h-full flex flex-col items-center justify-center text-slate-300 gap-2 font-black uppercase text-[9px]"><i data-lucide="construct" class="w-8 h-8 opacity-10"></i> ${cur.title} 모듈 준비 중</div>`;
        }
